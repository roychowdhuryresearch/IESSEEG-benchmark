#!/usr/bin/env python
"""Clip-level inference for a fine-tuned REVE model.
...
def main():
    ...
    model = ReveClassifier(args.pretrained_repo, n_classes=2)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            probs = torch.softmax(model(x, pos).float(), dim=-1)[:, 1].cpu().numpy()
