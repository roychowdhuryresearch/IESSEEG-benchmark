...
class Brain3DResNetTrainer:
    ...
    def train(self):
        ...
        # (D) Build DataLoaders. batch_size=64 (was 256): our wavelet
        # spectrograms have an unusually large time dimension (n_times =
        # epoch_length*sfreq = 6000 at 30s/200Hz), which the 3D ResNet only
        # downsamples gradually, so early-layer activations are large;
        # batch_size=256 needs >47GB and OOMs even alone on a 47GB GPU,
        # verified empirically -- batch_size=64 peaks at ~13GB, leaving
        # headroom to run alongside other jobs on a shared GPU.
        ...
        for epoch in range(max_epochs):
            ...
            # Check improvement. Require a minimum relative improvement
            # (min_delta) rather than any decrease at all -- on a small
            # validation set, val_loss is noisy enough that trivial
            # fluctuations were repeatedly resetting the patience counter
            # right as it was about to trigger, dragging training out much
            # longer than the model was actually still improving for.
