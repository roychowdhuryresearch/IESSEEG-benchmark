...
class VisionTransformer3D(nn.Module):
    ...
    def __init__(
        ...
        # Calculate sequence length based on the actual input dimensions and
        # patch size. Input shape is (batch, 1, n_eeg_channels, freq_seg,
        # time_samples) -- time_samples = epoch_length * sfreq varies with
        # the window length used (e.g. 2000 for 10s@200Hz, 6000 for
        # 30s@200Hz); this must be passed in to match, not hardcoded, or
        # pos_embed ends up the wrong size for the actual wavelet
        # spectrogram and the forward pass shape-mismatches.
        self.seq_len = (n_eeg_channels // patch_size[0]) * (freq_seg // patch_size[1]) * (time_samples // patch_size[2])
