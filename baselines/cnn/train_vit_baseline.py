...
class Brain3DViTTrainer:
    ...
    def train(self):
        ...
        # Build DataLoaders
        # Attention memory scales as num_heads * seq_len^2 (2993 tokens for a
        # 30s window here), not just batch*embed_dim like the CNN -- at
        # batch_size=64 this was OOMing (>13GB in a single attention layer
        # alone). Empirically verified: full fwd+bwd peak is ~21.9GB at
        # batch_size=16 and ~10.9GB at batch_size=8 on this architecture;
        # batch_size=8 leaves comfortable headroom on a shared GPU.
        ...
        # Build model. time_samples must match the actual wavelet
        # spectrogram's time dimension (epoch_length * sfreq), not a
        # hardcoded value -- otherwise pos_embed is sized for the wrong
        # sequence length and the forward pass shape-mismatches.
        time_samples = int(self.args.epoch_length * self.args.sfreq)
        model = vit3d(
        ...
        )  # note: `verbose` kwarg removed -- unsupported by newer PyTorch (>=2.x)
        ...
        for epoch in range(max_epochs):
            ...
            # Check improvement and save best model. Require a minimum
            # relative improvement (min_delta) rather than any decrease at
            # all -- on a small validation set, val_loss is noisy enough
            # that trivial fluctuations were repeatedly resetting the
            # patience counter right as it was about to trigger.
            if val_loss < best_val_loss * (1.0 - self.args.min_delta):
...
def main():
    ...
    parser.add_argument("--min_delta", type=float, default=0.02,
                        help="Minimum relative improvement in validation loss (fraction of best_val_loss) "
                             "required to reset the early-stopping patience counter and save a new best "
                             "checkpoint. Guards against noise on a small validation set repeatedly "
                             "resetting the counter right before patience would trigger.")
