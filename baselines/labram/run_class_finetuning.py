...
def get_args():
    ...
    parser.add_argument('--eval_freq', default=5, type=int,
                         help="Run Val+Test evaluation every N epochs (plus always on the "
                              "final epoch), instead of every epoch. Val+Test are forward-only "
                              "but still cost real wall-clock time; evaluating every single "
                              "epoch when only save_ckpt_freq epochs actually get a checkpoint "
                              "wastes most of that time on evaluations nothing will use.")
    ...
    parser.add_argument("--label_key", type=str, default="case_control_label",
                        help="Key to use for label column in training CSV.")
...
def get_dataset(args):
    if args.dataset == 'TUAB':
        ...
        # Any dataset tag other than the reference TUAB/TUEV placeholders
        # loads our own data via --train_csv/--data_root/--label_key. This
        # used to be an allowlist of six hardcoded 2-fold tag strings
        # (e.g. "A_case_vs_control"); generalized so arbitrary fold tags
        # (e.g. "case_control_fold0") work without needing to keep
        # enumerating every fold x task combination here.
        train_dataset, test_dataset, val_dataset = utils.prepare_ours_dataset(args.train_csv, args.data_root, args.label_key)
        ch_names = [
...
def main(args, ds_init):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        should_eval = data_loader_val is not None and (
